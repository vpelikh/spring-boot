...
When enabled, the plugin records the cache only when `bootBuildImage` is part of the same build execution:

* Injects `-XX:AOTCacheOutput=<path>` into all test task JVM arguments when `bootBuildImage` is scheduled
* Writes the cache file to `build/aot-cache/application.aot`
* Bundles the cache file into the built image
* **Fails** `bootBuildImage` with an actionable message if the cache file is not present, so you
...
A `test`-only run does not record the cache. A `bootBuildImage`-only run fails unless
`build/aot-cache/application.aot` exists (for example, left from a prior `./gradlew test bootBuildImage` run).
