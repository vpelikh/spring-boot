...
To enable AOT cache recording, bind the `aot-cache-record` goal to the `initialize` phase and set the
`spring-boot.aot-cache-record` property:
...
When enabled, the goal records the cache only when the `spring-boot:build-image` goal is part of the same
Maven execution:

* Injects `-XX:AOTCacheOutput=<path>` into the Surefire `argLine` when `build-image` is scheduled
* Writes the cache file to `target/aot-cache/application.aot`
* Bundles the cache file into the built image
* **Fails** `build-image` with an actionable message if the cache file is not present, so you
  cannot silently build an image without the cache

Run the tests together with the image build so the cache is recorded and then bundled in one execution:
...
A `mvn test`-only run does not record the cache. A `spring-boot:build-image`-only run fails unless
`target/aot-cache/application.aot` exists (for example, left from a prior `mvn test spring-boot:build-image` run).
