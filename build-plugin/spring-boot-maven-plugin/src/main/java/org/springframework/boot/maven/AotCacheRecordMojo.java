 ...
 *
 * <p>
 * Recording only happens when the {@code spring-boot:build-image} goal is part of the
 * same Maven execution, so tests record the cache only when the image is actually being
 * built.
 *
...
public class AotCacheRecordMojo extends AbstractMojo {
	...
	private boolean hasBuildImageGoal() {
		return hasCliBuildImageGoal() || hasBoundBuildImageGoal();
	}

	private boolean hasCliBuildImageGoal() {
		List<String> goals = (this.session != null && this.session.getGoals() != null) ? this.session.getGoals()
				: Collections.emptyList();
		return goals.stream().anyMatch((g) -> isBuildImageGoal(g));
	}

	private boolean hasBoundBuildImageGoal() {
		org.apache.maven.model.Build build = this.project.getModel().getBuild();
		if (build == null || build.getPlugins() == null) {
			return false;
		}
		return build.getPlugins()
			.stream()
			.filter((plugin) -> "spring-boot-maven-plugin".equals(plugin.getArtifactId()))
			.filter((plugin) -> plugin.getExecutions() != null)
			.flatMap((plugin) -> plugin.getExecutions().stream())
			.filter((execution) -> execution.getGoals() != null)
			.flatMap((execution) -> execution.getGoals().stream())
			.anyMatch((goal) -> isBuildImageGoal(goal));
	}

	private boolean isBuildImageGoal(String goal) {
		return goal.endsWith(":build-image") || goal.endsWith(":build-image-no-fork") || goal.equals("build-image")
				|| goal.equals("build-image-no-fork");
	}
