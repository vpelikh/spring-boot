...
public class CompositeTarArchive implements TarArchive {
	...
	public void writeTo(OutputStream outputStream) throws IOException {
		// The primary archive is written to an in-memory buffer so its entries can be
		// re-read and interleaved with the additional content into a single TAR layout.
		// The primary is an opaque TarArchive (not a Layout), so it cannot be streamed
		// into the destination directly; the whole primary is therefore held in memory
		// at once, which for a large application archive is a one-time, transient cost.
